...
def generate_input_target_stream(cxt, color_coh, correct_choice, n_t,
    ...
    # input: 3 dimensions
    # 0: left target; 1: right target; 2: color coherence
    ...
    # add a random noise
    coh_noise = np.random.normal(0, noise_factor, size=n_t)
    coh_noise[:dec_on] = 0
    input_stream[:,2] = input_stream[:,2] + coh_noise
